...
const WAKE_WORDS = ['星野'];
const GAOKAO_BRIDGE_ENDPOINT = process.env.GAOKAO_BRIDGE_ENDPOINT || 'http://127.0.0.1:8766';
...
`);
  ...
  if (enabled('gaokaoAdvisor')) {
    sections.push('📚 高考志愿顾问（私聊专用）\n[高考志愿] — 进入高考志愿咨询模式（与雪峰Agent多轮对话）\n[退出高考志愿] — 退出咨询模式\n');
  }
...
// Gaokao session tracking (userId → lastActive timestamp)
const gaokaoSessions = new Set<number>();
const gaokaoSessionTimes = new Map<number, number>();
const GAOKAO_SESSION_TIMEOUT_MS = 30 * 60 * 1000;
  ...
  newWs.on('message', async (data) => {
    ...
    try {
      ...
      if (event.post_type === 'message') {
        ...
        // ===== 高考志愿会话拦截（仅私聊） =====
        if (gaokaoSessions.has(userId) && !isGroup) {
          const lastTime = gaokaoSessionTimes.get(userId) || 0;
          if (Date.now() - lastTime > GAOKAO_SESSION_TIMEOUT_MS) {
            gaokaoSessions.delete(userId);
            gaokaoSessionTimes.delete(userId);
            try {
              await fetch(`${GAOKAO_BRIDGE_ENDPOINT}/gaokao/reset`, {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ user_id: userId }),
                signal: AbortSignal.timeout(3000)
              });
            } catch {}
            await sendMessage(userId, '高考志愿咨询已超时，如需继续请重新发送「高考志愿」~', '私聊');
            return;
          }

          if (lowerText === '退出高考志愿' || lowerText === '退出咨询' || lowerText === '结束咨询') {
            gaokaoSessions.delete(userId);
            gaokaoSessionTimes.delete(userId);
            try {
              await fetch(`${GAOKAO_BRIDGE_ENDPOINT}/gaokao/reset`, {
            ...
            await sendMessage(userId, '已退出高考志愿咨询模式，有需要随时找我~', '私聊');
            return;
          }
          // 转发消息到桥接服务
          gaokaoSessionTimes.set(userId, Date.now());

          try {
            const resp = await fetch(`${GAOKAO_BRIDGE_ENDPOINT}/gaokao/chat`, {
              method: 'POST',
            ...
            if (!resp.ok) {
              await sendMessage(userId, '高考志愿顾问暂时无法响应，请稍后再试~', '私聊');
              return;
            }

            const data = await resp.json() as any;
            // 发送完整推荐文件
            if (data.data_file && typeof data.data_file === 'string') {
              try {
                await sendActionWithResponse('upload_private_file', {
                  user_id: userId,
                  file: data.data_file,
                  name: '高考志愿推荐报告.txt'
                }, 15000);
              } catch (e: any) {
                console.error('[高考志愿] 文件上传失败:', e.message);
              }
            }
            await sendMessage(userId, data.reply || '(顾问暂无回复)', '私聊');
          } catch (ex: any) {
            console.error('[高考志愿] 桥接异常:', ex.message);
            const errMsg = ex.message.includes('fetch')
              ? '高考志愿顾问服务未连接，请联系管理员启动~'
              : `咨询异常: ${ex.message}`;
        ...
        // 高考志愿入口（私聊专用）
        if (lowerText === '高考志愿' || lowerText === '志愿咨询') {
          if (!isFeatureEnabled('gaokaoAdvisor')) {
            const msg = featureDisabledReply('高考志愿顾问');
          ...
          if (isGroup) {
            await sendMessage(groupId, '📚 高考志愿咨询仅限私聊使用，请加我好友后发送「高考志愿」开始~', '群聊');
            return;
          }

          if (gaokaoSessions.has(userId)) {
            gaokaoSessions.delete(userId);
            gaokaoSessionTimes.delete(userId);
            try {
              await fetch(`${GAOKAO_BRIDGE_ENDPOINT}/gaokao/reset`, {
          ...
          gaokaoSessions.add(userId);
          gaokaoSessionTimes.set(userId, Date.now());

          const welcomeMsg = [
            '📚 高考志愿咨询模式已开启！',
            '━━━━━━━━━━━━━━━━━━',
            '我是雪峰高考志愿顾问，直接告诉我你的情况：',
            '「我是浙江的，考了10500位，想学计算机」',
            '「河北 600分 临床医学」',
            '',
            '我会帮你分析学校和专业，给出冲稳保建议~',
            '发送「退出高考志愿」结束咨询',
          ].join('\n');
          await sendMessage(userId, welcomeMsg, '私聊');
          return;
        }
...
app.post('/api/shutdown', (req, res) => {
  const { execSync } = require('child_process');
  try {
    execSync('pm2 stop xingye-snowluma xingye-backend xingye-frontend xingye-gaokao && pm2 delete xingye-snowluma xingye-backend xingye-frontend xingye-gaokao', { stdio: 'ignore', timeout: 10000 });
